...
% raw non-standardized data were used
...
% 1. define Dir
covType='noGSR' % for gCor the analysis were performed on noGSR basic model

GroupAnalysis=['/home/data/Projects/workingMemory/results/CPAC_', preprocessDate, '/groupAnalysis/gCor'];
...
for j=1:numMeasure
    ...
    % full model
    labels={'age_demean', 'DS_FT_demean', 'DS_BT_demean', 'ageByFTdemean', 'ageByBTdemean', 'WASI_Full4IQ_demean', 'meanFDJenk_demean', 'hand_demean', 'gCor_demean', 'constant'}
    modelFull=[model(:, 14) model(:, 15) model(:, 16) model(:, 18) model(:, 19) model(:, 21) model(:, 22) model(:, 23) model(:, 26) ones(numSub, 1)];
    % if 2mm resolution use model(:, 26) for gCor; if 3mm resolution use model(:, 24) for gCor
    ...
    modelDF=[model(:, 14) model(:, 15) model(:, 18) model(:, 21) model(:, 22) model(:, 23) model(:, 26) ones(numSub, 1)];
    ...
    modelDB=[model(:, 14) model(:, 16) model(:, 19) model(:, 21) model(:, 22) model(:, 23) model(:, 26) ones(numSub, 1)];
    ...
    modelTot=[model(:, 14) model(:, 17) model(:, 20) model(:, 21) model(:, 22) model(:, 23) model(:, 26) ones(numSub, 1)];
