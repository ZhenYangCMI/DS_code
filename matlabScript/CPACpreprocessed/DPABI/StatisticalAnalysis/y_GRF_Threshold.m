...
if (~exist('dLh','var')) || (exist('dLh','var') && isempty(dLh))
    ...
    if strcmpi(Flag,'Z')
        ...
        if strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
            [Z_StatsImg P Header]=y_TFRtoZ(StatsImgFile,OutputName,Flag,Df1,Df2,Header);
            DOF=''; %Degree of freedom for residual files
            [dLh,resels,FWHM,nVoxels]=y_Smoothest(Z_StatsImg,MaskFile,DOF,VoxelSize);
        else
...
if ~strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
    y_Write(BrainVolume,[OutPath,filesep,'Z_ClusterThresholded_',OutName,OutExt],[nDim1,nDim2,nDim3],VoxelSize, Header,'double');
end
