...
function OverlayHeader=RedrawOverlay(SendHeader, curfig, curblob)
global st
if nargin<2
    curfig=gcf;
end
...
switch Value
    ...
    case 3 %Save Single Cluster
        ...
        Data=SingleCluster(OverlayHeader);
        if isempty(Data)
            return
        end
        Mask=logical(Data);
        y_Write(Data, OverlayHeader, fullfile(Path, [FileName, Ext]));
        y_Write(Mask, OverlayHeader, fullfile(Path, [MaskName, Ext]));
...
%             OverlayHeader.PMax);
    ...
    case 8 %Save colorbar as
        State=get(handles.ColorAxe, 'Visible');
        if strcmpi(State, 'On')
            [File, Path] = uiputfile({'*.tiff';'*.jpeg';'*.png';'*.bmp'},...
                'Save Picture As');
            if ~ischar(File)
            ...
            set(handles.ColorAxe, 'Visible', 'Off');
            BData=getframe(handles.ColorAxe);
            imwrite(BData.cdata, fullfile(Path, File));
            set(handles.ColorAxe, 'Visible', 'On');
        end
end
guidata(hObject, handles);
