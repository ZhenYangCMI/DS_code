...
%       subj.result.reproRank  : reproducibility rank of the aligned
%                                components
% Output:
...
for i = 1:size (subj.result.foundComp, 1)
    ...
    if max (subj.result.aveMap(i,:))== min (subj.result.aveMap(i,:)) || subj.result.reproRank(i) == 0
        subj.result.aveMap(i:end, :) = [];
        subj.result.aveTc(:,i:end) = [];
        subj.result.foundComp(i:end,:) = [];
        subj.result.reproRank(i:end) = [];
        break;
    end
